...
def createFromStringFunction(d, name):
    oe = outputEngine()
    oe.add("fun %s_fromstring_cont a s = \n" % name)
    oe.increaseIndent()
    oe.add('let \n') 
    oe.increaseIndent()
    oe.add('val s = case a of NONE => { readbuffer = s, writebuffer = growarray-new (chr 0), byte_offset = 0, bit_offset = 0 } \n')
    oe.add('                | SOME x => x \n')
    oe.add('val (r, s) =\n')
...
def createToStringFunction(d, name):
    oe = outputEngine()
    oe.add('fun %s_tostring_cont a r = \n' % name)
    oe.increaseIndent()
    oe.add('let \n')
    oe.increaseIndent()
    oe.add('val s = case a of NONE => { readbuffer = array(1, chr 0), writebuffer = growarray-new (chr 0), byte_offset = 0, bit_offset = 0 } \n')
    oe.add('                | SOME x => x \n')
    oe.add('val s =\n')
    ...
    oe.add('in\n')
    oe.increaseIndent()
    oe.add('(s, growarray-subarray ((#writebuffer/marshall_type s), 0, (#byte_offset/marshall_type s)))\n')
    oe.decreaseIndent()
    oe.add('end\n')
    oe.decreaseIndent()
