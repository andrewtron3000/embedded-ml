...
*)
        ...
        fun asm_to_string h = case h of 
                                ...
                                | CMP_LESSTHAN (l, r) => "(" ^ (asm_to_string l) ^ " < " ^ (asm_to_string r) ^ ")"
                                | CMP_LESSTHANEQ (l, r) => "(" ^ (asm_to_string l) ^ " <= " ^ (asm_to_string r) ^ ")"
                                | CMP_GREATERTHAN (l, r) => "(" ^ (asm_to_string l) ^ " > " ^ (asm_to_string r) ^ ")"
                                | CMP_GREATERTHANEQ (l, r) => "(" ^ (asm_to_string l) ^ " >= " ^ (asm_to_string r) ^ ")"
        ...
        fun generateMakefile bs = 
            let 
                ...
                val () = TextIO.output (f, ("CC := gcc\n"));
                val () = TextIO.output (f, ("CFLAGS := -O2\n"));
                val () = TextIO.output (f, ("AR := ar\n"));
                val () = TextIO.output (f, ("ARFLAGS := rS\n"));
                val () = TextIO.output (f, ("RM := rm -f \n"));
                val () = TextIO.output (f, ("RANLIB := ranlib\n"));
                val () = TextIO.output (f, ("OBJECTS := runtime-c.o main.o " ^ os ^ "\n"));
                val () = TextIO.output (f, ("a.out: $(OBJECTS)\n"));
                ...
                val () = TextIO.output (f, ("\t$(AR) $(ARFLAGS) libarchive.a $@\n"));
                val () = TextIO.output (f, ("#\t@$(RM) $@\n"));
                val () = TextIO.output (f, ("\n"));
            in
                TextIO.flushOut f;
                TextIO.closeOut f
      ...
      in
        ...
        TextIO.output (file, "   while (f != 0) {f = f();}\n");
